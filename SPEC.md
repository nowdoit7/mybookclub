...
2. **Orchestration in code** — a deterministic state machine (not the LLM) runs
   the 5-stage meeting, controls speaking order, and **forces meaningful
   examination**: after the user speaks, at least one persona must test the
   claim's evidence, scope, or consequence without inventing opposition.
...
5. **Participant or audience agency** — code schedules a genuine directed
   reader-to-reader examination, while bounded checkpoints let the user join,
...
| Quality of the idea | "Committed-stance multi-agent book club with enforced examination and honest disagreement" — not a chatbot skin. |
...
- **Meaningful position review**: ≥1 persona tests the user's stated position
  through disagreement, scope, evidence, causality, or consequence without
  inventing a conflict when the positions are compatible
...
- Base round (code-controlled):
  1. Alex explains the conversational thread and restates the selected topic as
     one natural spoken question without changing its meaning.
  2. `leadA` advances a committed claim from private notes and addresses `leadB`.
  3. `leadB` directly tests that claim, addresses `leadA`, and asks exactly one
     pointed question. When the positions conflict, the turn names the real
     disagreement; when they are compatible, it tests scope, evidence, or an
     unstated assumption without pretending to oppose `leadA`.
...
- The pre-checkpoint sequence above is a fixed engine invariant. There is exactly
  one directed AI-to-AI examination before the user chooses; `leadA` does not
  answer a third time.
...
- `join` asks the user for a position, extracts stance, and selects the persona
  furthest from that position as reviewer. A genuinely different position
  produces a direct challenge. When all stance distances are small, the reader
  acknowledges the compatible conclusion and tests its scope, evidence, or
  consequence instead of fabricating disagreement; Alex uses the devil's-advocate
  fallback only when the user passes without a position. The reviewer asks one
  pointed question, the next turn belongs to the user, and that same reviewer
  responds once so the exchange never breaks causally. Code then chooses the least-heard
...
- `wrap` or the extension cap carries the established shared ground and any real
  unresolved difference directly into WRAP_UP. Alex does not generate a duplicate
...
- Moderator gives a substantive, exactly 4-sentence spoken summary: central
  question, one precise user contribution, what genuinely shifted or converged,
  and any real unresolved counterclaim, then thanks the readers and says in the
...
## 9. Meaningful position review (anti-sycophancy) — core differentiator

The discussion opens with a directed persona-to-persona examination selected by
code. It may be a disagreement, a scope test, or an evidence test; the language
must reflect which one actually occurred.
...
`overallStance` only for ranking context; Stage 2 never triggers a position
review:

```
reviewer = persona whose notes.stanceByTopic[activeTopic]
           differs most from userStance
           (tie → random among most-distant; never the same persona twice in a row)

directive to reviewer:
  "The user just argued: {verbatim claim}. Compare that exact claim with your
   private position. If they conflict, name the precise disagreement. If they
   are compatible, acknowledge the shared conclusion and test one boundary,
   assumption, or missing piece of evidence. Ask one pointed question.
   2-4 sentences."
```

The pointed question opens a second user turn immediately. The reviewed line
and reviewer portrait remain visible while the user writes. After the user
answers, the same reviewer responds once, then a code-selected third reader
...
the selected reader tests the shared conclusion's scope or evidence without
pretending to disagree. The moderator plays devil's advocate only when the user
passes without stating a position. The guarantee is therefore: **the user's
position is always examined at least once, but false conflict is never required.**
...
- Disagree when your notes genuinely disagree. When positions are compatible,
  state the shared ground and test scope or evidence instead of inventing
  opposition. Concede only explicitly.
