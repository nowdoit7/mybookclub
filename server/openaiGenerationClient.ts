...
export class OpenAIGenerationClient implements GenerationClient {
  ...
  private async parse<T>(
    ...
    if (!response.output_parsed) {
      throw new ModelRefusalError(`The model did not return parsed ${schemaName} output.`);
    }
  ...
  async generateReadingNotes(input: ReadingNotesRequest) {
    return this.parse(
      ...
      `You are ${input.persona.name}. Stay committed to the supplied persona card. These are private anchor notes, not dialogue. Preserve every candidate topic verbatim and in order. overall_take must be 2-3 sentences. ${languageRule(input.language)} ${COPYRIGHT_RULE}`,
      JSON.stringify({
        book: input.book,
  ...
  async generateUtterance(input: UtteranceRequest) {
    ...
    const taskDirective =
      input.task === "TOPIC_OPEN"
        ? `State this exact code-selected question verbatim and do not substitute another topic: ${input.activeTopic}`
        : input.task === "CHALLENGE_USER"
          ? "Respectfully but firmly challenge the user's position and ask one pointed question; do not concede."
          : input.task === "CLOSING_REFLECTION"
            ? "State explicitly whether and how the discussion changed the view; do not claim movement that did not occur."
            : input.task === "DISCUSSION_SUMMARY"
              ? "Give a substantive spoken summary of the discussion: name the central disagreement, what shifted or remained unresolved, and bridge naturally to the written meeting recap. Base it only on the supplied conversation and do not introduce a new opinion."
            : "Perform the named task directly.";
    return this.parse(
      utteranceSchema,
      "table_utterance",
      `${
      ...
      } ${lengthRule} ${taskDirective} ${languageRule(input.language)} Mention persuasion only when the speaker's position genuinely changes, and acknowledge any change explicitly; otherwise do not use formulaic agreement. Shelf reference is ${
